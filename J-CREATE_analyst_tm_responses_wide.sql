...
Inputs from analyst schema: analyst.I_textmessages_tall

Outputs: analyst.J_tm_responses_wide
*/
use workflow;
DROP TABLE IF EXISTS analyst.J_tm_responses_wide;
CREATE TABLE analyst.J_tm_responses_wide as

with msg_order as (SELECT * FROM analyst.I_textmessages_tall)
...
, last_responses_v2 as (SELECT 
	*
    , CASE 
		WHEN last_response_text = 'PARAR' THEN 'OPT-OUT'
        WHEN last_response_text = 'SÍ' THEN 'YES'
        WHEN last_response_text in ('NO', 'OPT-OUT', 'YES') THEN last_response_text
        WHEN last_response_text is not null THEN 'OTHER'
        ELSE null
        END as last_response_categorical
